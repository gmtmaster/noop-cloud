# NOOP Sleep Need model (version 1)
...
Carried debt is updated chronologically:

```
deficit = max(0, need - main sleep)
repayment = max(0, main sleep - need) * 0.75
carried = clamp(previous carried * 0.90 + deficit - repayment, 0, 240 minutes)
