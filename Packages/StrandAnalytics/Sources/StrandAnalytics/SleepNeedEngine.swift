...
public enum SleepNeedEngine {
    ...
    public enum Configuration {
        ...
        public static let debtDecay = 0.90
        public static let repaymentFraction = 0.75
        public static let maximumCarriedDebtMinutes = 240.0
        public static let debtAdjustmentFraction = 0.50
        public static let maximumDebtAdjustmentMinutes = 120.0
    ...
    public static func calculate(priorMainSleepMinutes: [Double], priorEfficiencies: [Double],
        ...
        let debtAdjustment = min(max(0, carriedDebtMinutes) * Configuration.debtAdjustmentFraction,
                                 Configuration.maximumDebtAdjustmentMinutes)
    ...
    public static func history(_ inputs: [SleepNeedNightInput]) -> [SleepDebtHistoryPoint] {
        ...
        return inputs.map { input in
            ...
            let deficit = max(0, breakdown.totalSleepNeedMinutes - actual)
            let surplus = max(0, actual - breakdown.totalSleepNeedMinutes)
            let repayment = surplus * Configuration.repaymentFraction
            carriedDebt = min(Configuration.maximumCarriedDebtMinutes,
                              max(0, carriedDebt * Configuration.debtDecay + deficit - repayment))
