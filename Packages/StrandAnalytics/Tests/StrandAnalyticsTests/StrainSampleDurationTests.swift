...
final class StrainSampleDurationTests: XCTestCase {
    ...
    func testCleanOneHertzCadenceRetainsOneSecondPerSample() {
        let expected = Array(repeating: 1.0 / 60.0, count: 120)
        XCTAssertEqual(StrainScorer.sampleDurationsMinutes(uniform(120, stepS: 1)), expected)
    }

    func testBackfilledUnevenCadenceUsesEachActualGap() {
        let actual = StrainScorer.sampleDurationsMinutes(series(0, 1, 31, 61))
        XCTAssertEqual(actual, [1.0 / 60.0, 0.5, 0.5, 0.5])
    }

    func testRepeatedRescoringIsDeterministic() {
        let hr = series(0, 1, 31, 61, 3 * 3600)
        let first = StrainScorer.strain(hr, maxHR: maxHR, restingHR: rest, sex: "female")
        let second = StrainScorer.strain(hr, maxHR: maxHR, restingHR: rest, sex: "female")
