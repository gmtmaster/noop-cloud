...
final class ManualWorkoutRescoreRestingHrTests: XCTestCase {
    ...
    func testBelowEqualAndAboveSixtyMoveEffortInExpectedDirections() {
        let below = ManualWorkoutRescore.scored(windowSamples: window(), profile: profile,
                                                hrMax: hrMax, restingHR: 45)!.strain!
        let equal = ManualWorkoutRescore.scored(windowSamples: window(), profile: profile,
                                                hrMax: hrMax, restingHR: 60)!.strain!
        let above = ManualWorkoutRescore.scored(windowSamples: window(), profile: profile,
                                                hrMax: hrMax, restingHR: 75)!.strain!
        let implicitDefault = ManualWorkoutRescore.scored(windowSamples: window(), profile: profile,
                                                          hrMax: hrMax)!.strain!
        XCTAssertGreaterThan(below, equal)
        XCTAssertEqual(equal, implicitDefault)
        // 60 and 75 both place this fixture in Edwards zone 2, so the discretized model correctly ties.
        XCTAssertGreaterThanOrEqual(equal, above)
    }
